...
pub fn tool_registry() -> Vec<ToolMeta> {
    let mut tools = vec![
        ...
        ToolMeta {
            ...
            command: "ash clean [--profile <profile>] [--max-risk <risk>] --json".to_string(),
            category: "clean".to_string(),
            description: "Run the scan and apply phases in one call without bypassing plan verification or safety checks.".to_string(),
            parameters: vec![
                enum_param(
                    "profile",
                    "Optional scan profile. Defaults to the config default profile.",
                ...
                string_param(
                    "--app",
                    "Optional bundle id or .app path for targeted app cleanup planning.",
                    false,
                ),
                enum_param(
                    "--max-risk",
                ...
                bool_param("--dry-run", "Validate and report the chain without moving files.", false),
                string_param(
                    "--save-plan",
                    "Optional path to persist the generated plan JSON.",
                    false,
                ),
                bool_param(
                    "--include-plan",
                    "Include the full generated plan in the success payload.",
                    false,
                ),
            ],
            output_fields: vec![
            ...
            interactive_only: false,
            rate_limit: None,
            example: "ash clean --profile safe --dry-run --json".to_string(),
        },
        ToolMeta {
            ...
            parameters: vec![
                string_param(
                    "--plan",
                    "Path to a cleanup plan JSON file. If omitted, stdin is read and may contain a raw plan or the `ash scan --json` envelope.",
                    false,
                ),
                enum_param(
                    "--max-risk",
            ...
            input_schema: Some(BTreeMap::from([(
                "plan".to_string(),
                schema_field("object", "Cleanup plan JSON or a scan envelope containing data.plan"),
            )])),
            idempotent: false,
            read_only: false,
            supports_json: true,
            interactive_only: false,
            rate_limit: None,
            example: "ash scan --profile safe --json | ash apply --dry-run --json".to_string(),
        },
        ToolMeta {
