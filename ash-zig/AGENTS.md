...
- `bun run dev` - Build and run (debug)
- `bun run build` - Build binary to `zig-out/bin/ash`
- `bun run build:release` - Release build (ReleaseFast)
- `bun run test` - Run all tests
- `bun run check` - Format and test (quality gate)
- `bun run format` - Format code with zig fmt
...
- Quality gate: `bun run check` (format + test)
- Format: `zig fmt src/`
