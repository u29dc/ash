...
cargo run -q -p ash-cli -- clean --profile safe --dry-run --json >/dev/null
cargo run -q -p ash-cli -- apply --plan "$PLAN_FILE" --dry-run --json >/dev/null
cargo run -q -p ash-cli -- scan --profile safe --json | cargo run -q -p ash-cli -- apply --dry-run --json >/dev/null
